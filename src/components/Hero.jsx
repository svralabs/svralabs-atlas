...
import { fadeUp, fadeIn, fadeScaleIn, fadeLeftIn, btnHover } from '../lib/motion';

const HeroImage = "https://lh3.googleusercontent.com/aida-public/AB6AXuDY9j-qkt3jxDAWreIOqfRau1e5qiyvY1X2QJJrDesnbJW8363INHAy3pgt-6k3f3IG899HImhA15HjziDjsnyCt4UBrrcdGNldYWF4BkOjEAO4GvZssh0prOnjqxmYXJEI3Vki6Yh3h8ShHGQ7F--wmNXW35onNIoQGpbhFRxpJpaEqtXK9J6A1YMiru8NjLwOF9sK2gLKoWyVV-wKY_ykAld2gXA4hSnulS-DOLmAWJLrBXIlUkJ8pQ";
  ...
  return (
    <div className="w-full h-screen flex items-center justify-center p-3 md:p-5">
      <section className="relative w-full max-w-[1536px] h-full rounded-[1.5rem] md:rounded-[3rem] overflow-hidden flex flex-col items-center group">
        <img
          alt="Modern building corner with dark glass windows"
          className="absolute inset-0 w-full h-full object-cover object-center z-0"
          src={HeroImage}
        />
        <div
          className="absolute inset-0 z-[5]"
          style={{ background: 'linear-gradient(200deg, rgba(10,10,9,0.75) 0%, rgba(10,10,9,0.4) 50%, rgba(10,10,9,0.82) 100%)' }}
        />

        <div className="relative z-10 w-full h-full flex flex-col items-center">
          <div className="w-full flex flex-col items-center pt-10 px-6 text-center max-w-3xl" data-purpose="hero-copy">
            <motion.div
              className="flex items-center gap-2 px-4 py-2 rounded-full mb-4 w-fit"
              style={{ backgroundColor: 'var(--color-card-glass)', backdropFilter: 'blur(10px)', border: '1px solid var(--color-border)' }}
              initial="hidden"
              animate="visible"
              variants={fadeUp}
            >
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: 'var(--color-accent)' }}>
            ...
            <motion.h1
              className="text-4xl sm:text-5xl md:text-6xl lg:text-[78px] mb-3 tracking-tight leading-[1.05]"
              style={{ fontFamily: 'var(--font-display)', fontWeight: 400, color: 'var(--color-heading)' }}
              initial="hidden"
              animate="visible"
          ...
          <motion.div
            className="absolute bottom-28 right-4 left-auto md:left-6 md:right-auto md:bottom-6 lg:bottom-10 lg:left-10 p-3 md:p-4 lg:p-5 rounded-[1.2rem] md:rounded-[1.5rem] lg:rounded-[2.2rem] flex flex-col gap-2 lg:gap-3 min-w-[150px] w-fit"
            data-purpose="portfolio-status"
            style={{ backgroundColor: 'var(--color-card-glass)', backdropFilter: 'blur(20px)', border: '1px solid var(--color-border)' }}
            initial="hidden"
            animate="visible"
            variants={fadeLeftIn}
          >
            ...
            <button className="btn-hover flex items-center rounded-full pl-1.5 pr-4 py-1.5 gap-2 self-start" style={{ backgroundColor: 'var(--color-heading)', color: '#1C1808' }} {...btnHover}>
          ...
          {/* CornerNotch */}
          <motion.div
            className="absolute top-0 right-0 p-3 pb-5 pl-8 sm:p-4 sm:pb-6 sm:pl-10 md:p-6 md:pb-8 md:pl-14 rounded-bl-[1.5rem] sm:rounded-bl-[2rem] md:rounded-bl-[3.5rem] flex items-center gap-3 sm:gap-4 md:gap-6"
            data-purpose="latest-work-widget"
          ...
          >
            ...
            <div className="w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center" style={{ backgroundColor: 'rgba(201,162,39,0.1)', border: '1px solid rgba(201,162,39,0.3)' }}>
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: 'var(--color-accent)' }}>
                <path d="M7 17L17 7"></path>
                <path d="M7 7h10v10"></path>
              </svg>
            </div>
